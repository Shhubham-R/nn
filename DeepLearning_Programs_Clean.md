...
Perceptron from scratch to 
simulate OR, AND, and XOR 
logic gates, observing the 
iterative weight update process. 
Python, NumPy 
1.b 
Create a Multi-Layer Neural 
Network using Keras to 
replicate logical gates (solving 
the non-linear XOR 
classification problem). 
Python, Keras, TensorFlow 
2 
Implement a Convolutional 
Neural Network (CNN) 
architecture from scratch to 
classify the 10-class CIFAR-10 
image dataset. 
Python, TensorFlow, Keras 
3 
Design a Multi-Layer 
Perceptron (MLP) for 
handwritten digit classification 
using the classic MNIST 
dataset. 
Python, TensorFlow, Keras 
4 
Design a Simple Recurrent 
Neural Network (RNN) using 
Keras to model temporal 
patterns in a numeric sequence 
Python, Keras, NumPy 

and predict subsequent values. 
5 
Build a Long Short-Term 
Memory (LSTM) network model 
for sequence-based Sentiment 
Analysis using the IMDB movie 
review dataset. 
Python, TensorFlow, Keras 
6 
Implement a basic Generative 
Adversarial Network (GAN) 
architecture to synthesize 
realistic handwritten digits. 
Python, TensorFlow, Keras, 
Matplotlib 
7 
Implement a Saliency Map 
visualization technique to 
isolate and analyze high-
influence pixels driving a 
model's classifications. 
Python, TensorFlow 
(GradientTape), MobileNetV2 
 
 

PROGRAM 1.a: SINGLE-LAYER PERCEPTRON FROM SCRATCH 
Objective: Implement a single-layer artificial neuron from scratch utilizing fundamental NumPy matrix 
logic. The objective is to map linear separations and visually inspect iterative modifications to 
parameters across multiple evaluation iterations for logic truth tables (AND, OR, XOR). 
...
{labels[j]} ; Predicted: {predictions[j]} (w1: {weights[0]:.7f} & w2: 
{weights[1]:.7f})") 
...
Expected Output / Training History: 
...
Objective: Build a structural multilayered model with the Keras framework. The setup serves to 
demonstrate computational scaling limits of linear perceptrons on non-linearly separable truth 
functions like XOR, utilizing intermediate hidden layers. 
Source Code Implementation: 
import numpy as np 
...
PROGRAM 2: CONVOLUTIONAL NEURAL NETWORK (CNN) FOR CIFAR-10 
CLASSIFICATION 
Objective: Build a custom Convolutional Neural Network from scratch to process multi-channel 
feature frames. The target task centers on categorical sorting optimizations over the 10 separate 
visual target groups defining the CIFAR-10 database. 
Source Code Implementation: 
...
PROGRAM 3: MULTI-LAYER PERCEPTRON (MLP) FOR MNIST DIGIT 
CATEGORIZATION 
Objective: Construct a feedforward fully-connected network architecture utilizing modern 
optimization tools to categorize handwritten digital symbols from the MNIST benchmark dataset. 
Source Code Implementation: 
import tensorflow as tf 
...
PROGRAM 4: SIMPLE RECURRENT NEURAL NETWORK (RNN) FOR 
SEQUENCE MODELING 
Objective: Build a basic sequential Recurrent Neural Network topology tracking simple step 
distributions to extract and reproduce hidden pattern states in time-series records. 
Source Code Implementation: 
import numpy as np 
import tensorflow as tf 
from tensorflow.keras.models import Sequential 
...
print(f"Predicted Next Numerical Scalar : {prediction[0][0]:.2f}") 
print(f"Target Expectation Value  : 100.00") 
Expected Output / Training History: 
Training Simple RNN topology... (1000 epochs) 
 
Input Sequence           : [70, 80, 90] 
predicted next Number    : 100.00 
Target Number            : 100.00 
 
PROGRAM 5: LONG SHORT-TERM MEMORY (LSTM) FOR IMDB 
SENTIMENT CLASSIFICATION 
Objective: Implement an advanced gated recurrent Long Short-Term Memory structure tailored to 
ingest word vector strings for text sentiment categorization over movie review logs. 
Source Code Implementation: 
import tensorflow as tf 
from tensorflow.keras import layers, models 
...
Training ... 
epoch 1/3 
782/782 [......] - 121s 153ms/step - loss: 0.4755 - accuracy = 0.7740 - val_loss=0.3689 - 
val_acc=0.84 
epoch 2/3 
782/782 [.........] - 128s 164ms/step - loss = 0.3605 - accuracy = 0.8475 - val_loss = 
0.3536 - val_acc = 0.8410 
epoch 3/3 
782/782 [.........] - 23s 30ms/step - loss= 0.3271 - accuracy = 0.8635 
 
Total Text Accuracy : 86.35% 
 
PROGRAM 6: GENERATIVE ADVERSARIAL NETWORK (GAN) FOR DIGIT 
SYNTHESIS 
Objective: Formulate a twin adversarial layout composed of adversarial Generator and Discriminator 
neural networks to construct synthetic digital symbols matching baseline MNIST distributions. 
...
PROGRAM 7: SALIENCY MAPPING FOR MODEL INTERPRETABILITY 
Objective: Incorporate partial derivative tracking across model weights to map output gradients 
directly onto source image pixel grids, isolating key activation areas driving predictions. 
Source Code Implementation: 
import tensorflow as tf 
import numpy as np 
import matplotlib.pyplot as plt 
...
print(f"Top predicted class outcome classification: {decode_predictions(preds, 
top=1)[0]}") 
...
saliency = (saliency - tf.reduce_min(saliency)) / (tf.reduce_max(saliency) - 
...
Expected Output / Training History: 
1/1 [==============================] - 1s 540ms/step 
predicted: [('n02504458', 'African_elephant', 0.84231)] 
 
 
 

PROGRAM 8: ENCODER-DECODER SYSTEM FOR IMAGE CAPTIONING 
Objective: Design and implement an 'Encoder-Decoder' system where a CNN acts as the encoder 
(extracting image features) and an LSTM acts as the decoder (generating a descriptive sentence). 
import tensorflow as tf 
from tensorflow.keras import layers, models 
...
def build_encoder(): 
    ...
    encoder_model = models.Model(inputs=base_model.input,  
