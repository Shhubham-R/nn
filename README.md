...
**Aim:** Implement a Single Layer Perceptron from scratch using NumPy to simulate OR, AND, XOR logic gates and observe the weight update process.
...
**Aim:** Create a Neural Network using Keras to replicate the XOR logic gate problem. It is difficult to replicate exactly the Python code in the previous example using Keras.
...
**Aim:** Implement a CNN architecture from scratch to classify the CIFAR-10 dataset.
...
**Aim:** Design a Multi-Layer Perceptron (MLP) for handwritten digit classification (MNIST) using Keras.
...
**Aim:** Design and implement a Simple Recurrent Neural Network (RNN) using Python and Keras to learn the temporal pattern of a numeric sequence and predict the subsequent value in that sequence. Demonstrate the model's performance on a simple arithmetic progression.
...
# We define sequences of 3 numbers (X) to predict the 4th number (y)
...
model = Sequential([
    # 128 units: Number of hidden neurons
    # input_shape=(3, 1): 3 time steps, 1 feature per step
    SimpleRNN(128, activation='relu', input_shape=(3, 1)),

    # Dense layer: To output the single predicted value
    Dense(1)
])

# Step 3: Compile the Model
# We use Mean Squared Error (MSE) because this is a regression problem
...
# Step 5: Test with a new sequence [70, 80, 90]
...
**Aim:** Build an LSTM model for Sentiment Analysis using the IMDB dataset.
...
# We only take the top 10,000 most frequent words to keep the model efficient
max_features = 10000
...
# Reviews have different lengths; we pad them so they all have a length of 200
x_train = sequence.pad_sequences(x_train, maxlen=maxlen)
x_test  = sequence.pad_sequences(x_test,  maxlen=maxlen)

# 3. Build the LSTM Model
model = models.Sequential([
    # Turns word integers into dense vectors of fixed size (128)
    layers.Embedding(max_features, 128, input_length=maxlen),

    # The LSTM layer allows the model to remember context in the sequence
    layers.LSTM(64, dropout=0.2, recurrent_dropout=0.2),

    # Final output layer: Sigmoid maps result to 0 (negative) to 1 (positive)
...
**Aim:** Implement a basic Generative Adversarial Network (GAN) to generate synthetic digits.
...
# This matches the 'tanh' activation function used in the generator
x_train = (x_train.astype('float32') - 127.5) / 127.5

# Reshape to (28, 28, 1) to represent grayscale channel
x_train = np.expand_dims(x_train, axis=-1)

# --- 2. BUILD THE GENERATOR ---
def build_generator():
    model = tf.keras.Sequential([
        # Take 100 random numbers (noise) as input
        layers.Dense(128, input_dim=100, activation='relu'),
        layers.Dense(256, activation='relu'),
        # Output 784 pixels (28x28) scaled between -1 and 1
        layers.Dense(784, activation='tanh'),
        # Reshape the flat vector back into an image format
...
def build_discriminator():
    model = tf.keras.Sequential([
        ...
        layers.Dense(256, activation='relu'),
        layers.Dense(128, activation='relu'),
        # Final neuron outputs a probability (0 = Fake, 1 = Real)
        layers.Dense(1, activation='sigmoid')
    ])
    model.compile(optimizer='adam',
                  loss='binary_crossentropy',
...
# Connect: Noise -> Generator -> Fake Image -> Discriminator -> Decision
...
**Aim:** Implement a Saliency Map to visualize which pixels influence a model's prediction.
...
**Aim:** Design and implement an "Encoder-Decoder" system where a CNN acts as the encoder (extracting image features) and an LSTM acts as the decoder (generating a descriptive sentence).
...
def build_encoder():
    ...
    # Extract features from the second-to-last layer (Global Average Pooling)
...
def build_caption_model(vocab_size, max_length):
    ...
    # Final Output Layer (Predicts the next word in vocabulary)
